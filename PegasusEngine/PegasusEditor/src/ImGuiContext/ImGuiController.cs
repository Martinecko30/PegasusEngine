...
namespace PegasusEngine.PegasusEditor.ImGuiContext;
...
public class ImGuiController : IDisposable
{
    ...
    public ImGuiController(int width, int height)
    {
        ...
        unsafe void LoadTextFont(Fonts name, string sourcePath, float fontSizePx)
        {
            Log.EditorInfo($"Using text font file: {sourcePath}");
            ...
            IntPtr ranges = io.Fonts.GetGlyphRangesDefault();
            
            var font = io.Fonts.AddFontFromFileTTF(sourcePath, fontSizePx, cfg, ranges);
            ...
            ImGuiFonts.Register(name, font);
        }
        
        unsafe void MergeIconFont(string sourcePath, float fontSizePx)
        {
            Log.EditorInfo($"Merging icon font: {sourcePath}");
            
            var iconsConfig = ImGuiNative.ImFontConfig_ImFontConfig();
            iconsConfig->MergeMode = 1;
            iconsConfig->PixelSnapH = 1;
            iconsConfig->GlyphMinAdvanceX = fontSizePx;
            ...
            fixed (ushort* iconRangesPtr = iconRanges)
            {
                io.Fonts.AddFontFromFileTTF(sourcePath, fontSizePx, iconsConfig, (IntPtr)iconRangesPtr);
            }
